...
u_int64_t tfs_get_file_size(char *file_name) {
    inode_t *root_dir_inode = inode_get(ROOT_DIR_INUM);
    int inum = tfs_lookup(file_name, root_dir_inode);
    ALWAYS_ASSERT(inum != -1, "File system error.");
    return inode_get(inum)->i_size;
}

int tfs_create_file(char *file_name) {
    inode_t *root_dir_inode = inode_get(ROOT_DIR_INUM);
    ALWAYS_ASSERT(root_dir_inode != NULL, "tfs_create_file: root dir inode must exist");
    int inum = inode_create(T_FILE);
    if (inum == -1) {
        return -1; // no space in inode table
    }
    if (add_dir_entry(root_dir_inode, file_name, inum) == -1) {
        inode_delete(inum);
        return -1; // no space in directory
    }
    return 0;
