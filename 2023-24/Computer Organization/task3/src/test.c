...
int main() {
    ...
    { // Basic read/write test
        ...
        write(Address, (uint8_t *)(&WriteVal));
        uint32_t time1 = getTime();

        read(Address, (uint8_t *)(&ReadVal));
        uint32_t time2 = getTime();

        uint32_t expect1 = L1_WRITE_TIME + L2_READ_TIME + DRAM_READ_TIME;
    ...
    //         testvalues[i] = i * 2;
    //         write(i * conflictStride, (uint8_t *)(&testvalues[i]));
    //     }
    ...
    { // Testing associativity values
        ...
        for (uint32_t i = 0; i < L1_ASSOCIATIVITY + 1; i++) {
            uint32_t temp;
            read(i * conflictStride, (uint8_t *)&temp);
