...
Cypress.Commands.add('deleteAllButArs', () => {
  ...
  cy.task('queryDatabase', {
    ...
    cy.task('queryDatabase', {
        query: "DELETE FROM USER_DOCUMENT",
        credentials: credentials,
    });
  cy.task('queryDatabase', {
...
Cypress.Commands.add('createAssessments', ()=> {
    cy.exec('psql -U ' + credentials.user + ' -d hedb -h ' + credentials.host + ' -p ' + credentials.port + ' < ../data/database-initialization/hedb-create-assessments.sql', {env: {
